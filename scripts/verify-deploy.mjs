...
async function verifyDeployment(maxAttempts = 8, delayMs = 6000) {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    process.stdout.write(
      `⏳ Попытка ${attempt}/${maxAttempts}: опрос ключевых страниц... `,
    );
    ...
    if (failed.length === 0) {
      ...
      results.forEach((r) => {
        ...
        console.log(
          `  ✓ 200 OK: ${path.padEnd(28)} (${Math.round(r.size / 1024)} KB)`,
        );
      });
      console.log("────────────────────────────────────────────\n");
      return true;
    ...
    if (attempt < maxAttempts) {
      console.log(
        `Vercel ещё собирает проект (ошибок: ${failed.length}). Ждём ${delayMs / 1000}с...`,
      );
      ...
      failed.forEach((f) =>
        console.log(
          `  ❌ ${f.url} — статус: ${f.status} (${f.error || "Не ответил"})`,
        ),
      );
      return false;
    }
