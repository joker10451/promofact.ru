...
const INTRO_TEMPLATES = [
  (name: string, n: number, cw: string, disc: string, cat: string, monthGen: string) =>
    `Ищете промокоды ${name}? На ${monthGen} мы собрали ${n} ${cw} на ${cat} со скидкой ${disc}. Все коды получены из официальных партнерских программ и действуют на момент публикации — копируйте и применяйте при оформлении заказа.`,
  ...
  (name: string, n: number, cw: string, disc: string, cat: string, monthGen: string) =>
    `Чтобы не переплачивать в ${name}, используйте промокоды на ${cat} из таблицы ниже. На ${monthGen} доступно ${n} ${cw}, максимальная скидка — ${disc}. Истёкшие предложения убираются из подборки автоматически.`,
  ...
  const faq: FAQItem[] = [
    {
      ...
      answer: `На ${monthGen} доступно ${n} ${cw} ${store.name} со скидкой ${disc}. Все предложения актуальны и получены из официальной партнерской программы.`,
    },
    {
