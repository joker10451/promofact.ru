...
export async function generateMetadata({
  ...
  return {
    ...
    // отфильтрованные по типу заказа. Они составляли половину карты сайта, и
    // робот выгребал их первыми, оставляя категориям остаток бюджета обхода.
    // В индексе при этом не давали ничего. Закрыты от индексации, но follow
    // сохранён: ссылки с них на магазины и категории продолжают работать.
...
export default async function StoreFirstOrderPage({
  ...
  const faqItems = [
    ...
    },
    {
      ...
      a: `На ${monthYear} выгода на первый заказ в ${store.name} достигает ${maxDisc}. Все предложения получены напрямую от партнерской программы.`,
    },
  ];
